...
/*
		...
		if ( strcmp(root->Value(),"Start") == 0 ){
			Ogre::SharedPtr<Start> start(new Start(root));
		...
		if ( strcmp(root->Value(),"StartPosition") == 0 ){
			Ogre::SharedPtr<StartPosition> startp(new StartPosition(root));
			for ( std::list<UserDataCallback*>::iterator uc = mUserDataCallback.begin(); uc != mUserDataCallback.end(); uc++ ){
		...
		//FINISH
			...
			Ogre::SharedPtr<Finish> finish(new Finish(root));
			for ( std::list<UserDataCallback*>::iterator uc = mUserDataCallback.begin(); uc != mUserDataCallback.end(); uc++ ){
				(*uc)->onFinish(finish);
			}
		}
		//CHECKPOINT
			...
			Ogre::SharedPtr<CheckPoint> checkpoint(new CheckPoint(root));
			for ( std::list<UserDataCallback*>::iterator uc = mUserDataCallback.begin(); uc != mUserDataCallback.end(); uc++ ){
		...
		//ASTEROID
			...
			Ogre::SharedPtr<Asteroid> ast(new Asteroid(root));
			for ( std::list<UserDataCallback*>::iterator uc = mUserDataCallback.begin(); uc != mUserDataCallback.end(); uc++ ){
				(*uc)->onAsteroid(ast);
			}
		}
		//HOVERCRAFT
			...
			Ogre::SharedPtr<Hovercraft> hovercr(new Hovercraft(root));
			for ( std::list<UserDataCallback*>::iterator uc = mUserDataCallback.begin(); uc != mUserDataCallback.end(); uc++ ){
		...
		//BOOST
			...
			Ogre::SharedPtr<Boost> boost(new Boost(root));
			for ( std::list<UserDataCallback*>::iterator uc = mUserDataCallback.begin(); uc != mUserDataCallback.end(); uc++ ){
				(*uc)->onBoost(boost);
			}
		}
		//PORTAL
			...
			Ogre::SharedPtr<Portal> portal(new Portal(root));
			for ( std::list<UserDataCallback*>::iterator uc = mUserDataCallback.begin(); uc != mUserDataCallback.end(); uc++ ){
				(*uc)->onPortal(portal);
			}
		}
		//POWERUPSPAWN
			...
			Ogre::SharedPtr<PowerupSpawn> spawn(new PowerupSpawn(root));
			for ( std::list<UserDataCallback*>::iterator uc = mUserDataCallback.begin(); uc != mUserDataCallback.end(); uc++ ){
		...
		//RESETSPAWN
			...
			Ogre::SharedPtr<ResetSpawn> spawn(new ResetSpawn(root));
			for ( std::list<UserDataCallback*>::iterator uc = mUserDataCallback.begin(); uc != mUserDataCallback.end(); uc++ ){
		...
		//TRACK
			...
			Ogre::SharedPtr<Track> track(new Track(root));
			for ( std::list<UserDataCallback*>::iterator uc = mUserDataCallback.begin(); uc != mUserDataCallback.end(); uc++ ){
				(*uc)->onTrack(track);
			}
		}*/

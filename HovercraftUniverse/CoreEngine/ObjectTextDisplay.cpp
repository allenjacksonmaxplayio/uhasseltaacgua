...
namespace HovUni {
	...
	void ObjectTextDisplay::update()  {
		...
		Ogre::Vector3 position = bbox.getCenter();
		Ogre::Vector3 cameraPos = m_c->getPosition();;
		Ogre::Vector3 forward = cameraPos + m_c->getDirection();
		forward.normalise();
		Ogre::Vector3 camera_to_object = bbox.getCenter() - cameraPos;
		*/
		
		//Project camera_to_object onto the forward vector
		//Check if the object is in front of our camera
		//if (forward.directionEquals(camera_to_object, Ogre::Radian(Ogre::Math::PI))) {
